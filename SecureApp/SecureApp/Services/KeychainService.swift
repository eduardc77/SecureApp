...
public class KeychainService {
	public static let credentialsStorageKey = "credentials"

	static func getCredentials() -> Credentials? {
		guard let credentialsString: String = value(forKey: credentialsStorageKey) else { return nil }

		return Credentials.decode(credentialsString)
	}

	static func saveCredentials(_ credentials: Credentials) -> Bool {
		guard set(credentials.encoded(), forKey: credentialsStorageKey) else { return false }
	...
	public class func allValues() -> [[String: String]]?  {
		...
		guard status == errSecSuccess, let attributeDicts = retrievedAttributes as? [[String: AnyObject]] else { return nil }
